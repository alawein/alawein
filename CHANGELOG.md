...
- Ops glossary for `Sync [project]` vs `GitHub Sync`, canonical `projects.json`
  -> Notion flow, per-repo scan vs Notion push, and the related architecture
  guardrails now lives in the private Alawein control plane.
...
- Portfolio **`portfolio_domain`** on `projects.json` featured entries (Notion Domain select). **`notion_sync`** array for Notion-only rows (qmlab, simcore, meatheadphysicist) without expanding README. The detailed Notion database guide now lives in the private Alawein control plane.
- Local Notion validate + sync + canonical verification helpers now live in the
  private Alawein control plane alongside the private Notion sync workflow.
