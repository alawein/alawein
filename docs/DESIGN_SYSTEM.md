...
AlaweinOS uses a token-based design system defined in `src/index.css` and extended in
`tailwind.config.ts`.
...
```tsx
import { NeuCard, NeuButton, NeuSlider, NeuToggle } from '@/components/design-engines/neumorphism';
...
```tsx
import { BrutalCard, BrutalButton, BrutalTable } from '@/components/design-engines/brutalist';
...
```tsx
import { PastelCard, PastelButton, PastelBadge } from '@/components/design-engines/soft-pastel';
