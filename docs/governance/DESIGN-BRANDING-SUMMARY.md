...
**Done (2026-03-13):** simcore PR #6 merged; main pulled. **Vercel deploy — OK:** simcore, devkit, llmworks, qmlab, bolts, gainboy, rounaq-atelier, event-discovery-framework, **repz**, **meshal-web**. meshal-web: pinned @clerk/react to 6.1.0 and use Show (fixes loadClerkUiScript build error); redeploy succeeded. **attributa:** installCommand added, pushed; install still fails with 128 (has `github:alawein/alawein-ui` dep — Vercel may need token or published package).
