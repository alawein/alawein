...
A repository has exactly one version anchor: the **git tag** and the top
**CHANGELOG** entry are canonical and must agree. Every other version string is
derived from that anchor and must not be edited independently:

- `pyproject.toml` / `package.json` `version` derive from the canonical anchor.
- A package `__init__.py` `__version__` derives from the manifest.
- CLI `--version` output derives from the same source.

Per-document `version:` frontmatter is discouraged because it only drifts; omit
it unless a document genuinely versions independently of the repository. When
the anchors disagree, the repository is in an inconsistent release state. The
`version-coherence.py` check compares the manifest version, the package
`__version__`, the top CHANGELOG entry, and the latest git tag, and warns on any
mismatch.
...
| Logs | Generated | Runtime | No | None | n/a | Ephemeral, not reproducible |
| Reports | Generated | Pipelines | No | None | n/a | Point-in-time snapshots |
| Snapshots | Frozen | Prior canonical | No | None | n/a | Immutable after creation |
...
| projects.json | Derived from `catalog/repos.json` (org repo only) | Regenerated by `build-catalog.py`; never duplicated to product repos | Hand-edit or local copy -> regenerate or delete |
