...
### Shared packages (Alawein devkit)
...
**format / lint / type-check** (and optionally **`build`** with `--with-build`)
against the workspace clones managed by the private Alawein control plane:
