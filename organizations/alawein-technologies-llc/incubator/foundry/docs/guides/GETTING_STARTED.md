...
2. **Strategic planning** → `docs/analysis/` (comprehensive analysis)
3. **Implementation roadmaps** → `docs/roadmaps/` (week-by-week plans)
4. **Repository templates** → `products/` (starter code for each idea)
...
```bash
cat docs/analysis/PRIORITY_RANKING.md
```
...
```bash
cat docs/roadmaps/PHASE_1_QUICK_WINS.md
```
...
# Copy template
cp -r /path/to/crazy-ideas/products/nightmare-mode/* .
...
1. **Check the docs** in `docs/` first
