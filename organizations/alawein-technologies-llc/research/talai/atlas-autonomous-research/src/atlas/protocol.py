...
class ATLASProtocol:
    ...
    async def run_qap_benchmark_with_turingo(
        self,
        instance: str,
        *,
        ...
        """Optional helper: run a QAP benchmark via Turingo.

        This uses the tal-ai level optimization_client facade when available.
        It does not affect the main research pipeline; callers can use this to
        gather additional empirical evidence for optimization-focused
        hypotheses.
        """

        try:
            from optimization_client import HAS_TURINGO, turingo_async_solve
        except ImportError as exc:  # pragma: no cover - optional dependency
            raise ImportError(
                "optimization_client module not available. Ensure the tal-ai "
                "workspace (with optimization_client.py) is on PYTHONPATH."
            ) from exc

        if not HAS_TURINGO:
            raise ImportError(
                "Turingo is not available. Install or expose the Turingo "
                "project with its turingo_client before calling this helper."
            )

        return await turingo_async_solve(
            problem_type="qap",
            instance=instance,
            paradigms=paradigms,
    ...
    def run_qap_with_Librex(
        ...
        *,
        ...
        """Optional helper: solve a QAP instance via Librex.

        Delegates to optimization_client.optimize_qap when available. This is
        intended for lightweight experiments or unit tests rather than being
        wired into the main ORCHEX pipeline by default.
        """

        try:
            from optimization_client import HAS_Librex, optimize_qap
        except ImportError as exc:  # pragma: no cover - optional dependency
            raise ImportError(
                "optimization_client module not available. Ensure the tal-ai "
                "workspace and Librex repo are on PYTHONPATH."
            ) from exc

        if not HAS_Librex:
            raise ImportError(
                "Librex is not available. Install Librex and its "
                "Librex_client before calling this helper."
            )
