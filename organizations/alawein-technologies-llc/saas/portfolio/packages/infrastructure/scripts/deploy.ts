...
class RepzDeployer {
  ...
  private async deployApplication(): Promise<void> {
    ...
    switch (this.config.provider) {
      case 'aws':
        ...
        exec(
          `aws ecs update-service --cluster repz-${this.config.environment} --service repz-app --force-new-deployment`
        );
        break;
      case 'azure':
        // Deploy to Azure Container Instances or App Service
        exec(
          `az webapp deployment source sync --resource-group repz-${this.config.environment} --name repz-app`
        );
        break;
      case 'gcp':
        // Deploy to Cloud Run or GKE
        exec(
          `gcloud run deploy repz-app --image gcr.io/${process.env.GCP_PROJECT}/repz-platform:${this.config.version}`
        );
        break;
      case 'docker':
  ...
  private async deployAwsInfrastructure(): Promise<void> {
    ...
    // Plan deployment
    exec(`terraform plan -var="environment=${this.config.environment}" -out=tfplan`, {
      cwd: terraformDir,
    });
  ...
  private async deployGcpInfrastructure(): Promise<void> {
    ...
    exec(
      `gcloud deployment-manager deployments create repz-${this.config.environment} --config infrastructure.yaml`
    );
  }
  ...
  private async deployDockerInfrastructure(): Promise<void> {
    ...
    exec(
      'docker-compose -f packages/infrastructure/docker/docker-compose.yml up -d postgres redis minio'
    );
  }
  ...
  private async generateDeploymentManifests(): Promise<void> {
    ...
    writeFileSync(`deployment-manifest-${this.config.environment}.yaml`, yaml.stringify(manifest));
  }
...
program.name('deploy').description('REPZ Platform deployment orchestrator').version('1.0.0');
