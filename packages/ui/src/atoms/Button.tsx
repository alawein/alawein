  ...
  ],
  {
    variants: {
      ...
      size: {
        ...
        md: ['h-10 px-4 py-2', 'min-width: var(--size-touch-target-md)', 'gap: var(--spacing-sm)'],
        lg: [
          'h-11 px-8 text-base',
        ...
        icon: ['h-10 w-10', 'min-width: var(--size-touch-target-md)'],
      },
      loading: {
        true: 'cursor-wait',
        false: '',
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, loading, className }), fullWidth && 'w-full')}
        ref={ref}
        disabled={isDisabled}
        aria-busy={loading}
        {...props}
