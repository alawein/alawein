...
const importReplacements = [
  ...
  {
    pattern: /from ['"]\.\.\/\.\.\/lib\/performance-optimization['"]/g,
    replacement: `from '@alawein/utils/performance'`,
  },
...
async function migrateProject(projectPath: string): Promise<MigrationStats> {
  ...
  try {
    ...
    console.log(
      `  ✅ Updated ${stats.filesUpdated} files, replaced ${stats.importsReplaced} imports, deleted ${stats.filesDeleted} files`,
    );
  } catch (error) {
