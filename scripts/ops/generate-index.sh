...
generate_index() {
  ...
  # Derive last_updated from docs history in UTC, matching CI's date boundary.
  # Regeneration stays stable when docs content has not changed.
  local last_updated docs_rel existing_last_updated
  docs_rel="${docs_dir#${repo_root}/}"
  last_updated=$(TZ=UTC git -C "$repo_root" log -1 --format=%cd --date=format-local:%F -- "$docs_rel" 2>/dev/null || true)
