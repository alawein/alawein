  ...
  validateDocumentId(filePath, metadata) {
    ...
    if (documentId) {
      ...
      if (!idPattern.test(documentId)) {
        this.addError(filePath, 'Invalid document ID', `Document ID should match pattern: XXX-XXX-001 (current: ${documentId})`);
      }
    }
  }
  ...
  validateVersion(filePath, metadata) {
    ...
    if (version) {
      ...
      if (!versionPattern.test(version)) {
        this.addError(filePath, 'Invalid version', `Version should follow semantic versioning: X.Y.Z (current: ${version})`);
      }
    }
  }
  ...
  validateStatus(filePath, metadata) {
    ...
    if (status) {
      ...
      if (!validStatuses.includes(status)) {
        this.addError(filePath, 'Invalid status', `Status must be one of: ${validStatuses.join(', ')} (current: ${status})`);
      }
    }
  }
  ...
  validateClassification(filePath, metadata) {
    ...
    if (classification) {
      ...
      if (!validClassifications.includes(classification)) {
        this.addError(filePath, 'Invalid classification', `Classification must be one of: ${validClassifications.join(', ')} (current: ${classification})`);
      }
    }
  }
  ...
  validateDates(filePath, metadata) {
    ...
    if (dates) {
      ...
      if (dates.next_review) {
        ...
        if (nextReview <= today) {
          this.addWarning(filePath, 'Review date passed', `next_review date (${dates.next_review}) is in the past or today`);
        }
      }
    }
  }
